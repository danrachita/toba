...
//Edit a entry from AdrenalineNode(masternodeConfig)
//Added at B_CLIENT_VERSION 1.1.0.0
void MasternodeManager::on_tableWidget_2_doubleClicked(const QModelIndex &index)
{
...
void MasternodeManager::on_editButton_clicked()
{
    ...
    AddEditAdrenalineNode* aenode = new AddEditAdrenalineNode(sAlias);
    aenode->exec();
    on_UpdateButton_clicked();
}


//Remove a entry from AdrenalineNode(masternodeConfig)
//Added at B_CLIENT_VERSION 1.1.0.0
void MasternodeManager::on_removeButton_clicked()
{
    ...
    confirm = QMessageBox::question(this, "Remove Master Node?", "Are you sure you want to delete this master node configuration?", QMessageBox::Yes|QMessageBox::No);
    if(confirm == QMessageBox::No)    return;

    // remove the node from masternodeConfig
    QModelIndex index = selected.at(0);
    int r = index.row();
    std::string sAlias = ui->tableWidget_2->item(r, 0)->text().toStdString();

    if (masternodeConfig.remove(sAlias)) {
