...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeTobaAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTobaAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        //2018-02-03 BUG Fix
        //strAnonymizeTobaAmount = strAnonymizeTobaAmount.remove(strAnonymizeTobaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAnonymizeTobaAmount = strAnonymizeTobaAmount.remove(strAnonymizeTobaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit));
        strAmountAndRounds = strAnonymizeTobaAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeTobaAmount*COIN) nMaxToAnonymize = nAnonymizeTobaAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeTobaAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeTobaAmount));
        //2018-02-03 BUG Fix
        //strAnonymizeTobaAmount = strAnonymizeTobaAmount.remove(strAnonymizeTobaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAnonymizeTobaAmount = strAnonymizeTobaAmount.remove(strAnonymizeTobaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit));
        strAmountAndRounds = strAnonymizeTobaAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
