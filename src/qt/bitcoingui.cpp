...
{
    resize(900, 520);
    setWindowTitle(tr("Toba") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("toba");
    setStyleSheet("#toba { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #dedede, stop: 1 #efefef);  }");
    // Accept D&D of URIs
    setAcceptDrops(true);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Toba address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/toolbar"), tr("&About TOBA"), this);
    aboutAction->setToolTip(tr("Show information about TOBA"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Toba"));
...
void BitcoinGUI::createToolBars()
{
    ...
    toolbar->setStyleSheet("QToolButton { color: #ffffff; font-size: 13px; font-weight: 400; font-family: 'Verdana'; padding: 5px; padding-left: 8px; border: none; }" 
                           ...
                           "#tabs { color: #000000; background-color: #eb2123; border: none; padding-top: 0px; padding-bottom: 0px; }");
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Toba client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Toba client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Toba network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Toba address or malformed URI parameters."));
    }
