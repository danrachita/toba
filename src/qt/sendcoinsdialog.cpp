    ...
    fNewRecipientAllowed(true)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Toba address (e.g. C8gZqgY4r2RoEdqYk3QsAqFckyf9pRHN6i)"));
#endif
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    {
        CTOBAcoinAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::IXTransactionCreationFailed:
        msgParams.first = tr("InstantX doesn't support sending values that high yet. Transactions are currently limited to 1000 TOBA.");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CTOBAcoinAddress addr = CTOBAcoinAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Toba address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
