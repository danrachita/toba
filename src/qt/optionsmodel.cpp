...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTobaAmount"))
        settings.setValue("nAnonymizeTobaAmount", 1000);
    nAnonymizeTobaAmount = settings.value("nAnonymizeTobaAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeTobaAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeTobaAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeTobaAmount:
            return QVariant(nAnonymizeTobaAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeTobaAmount:
            nAnonymizeTobaAmount = value.toInt();
            settings.setValue("nAnonymizeTobaAmount", nAnonymizeTobaAmount);
            emit AnonymizeTobaAmountChanged(nAnonymizeTobaAmount);
            break;
        default:
            break;
        }
