...
//
// Bootup the masternode, look for a 50000 TOBA input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in toba.conf */
...
bool CActiveMasternode::GetMasterNodeVin(CTxIn& vin, CPubKey& pubkey, CKey& secretKey, std::string strTxHash, std::string strOutputIndex) {
	...
	if(!strTxHash.empty()) {
		...
		int outputIndex;
		try {
			outputIndex = boost::lexical_cast<int>(strOutputIndex);
		}
		catch (std::exception &e) {
			LogPrintf("CActiveMasternode::GetMasterNodeVin - Invalid strOutputIndex\n");
			return false;
		}
...
{
    CTOBAcoinAddress address(collateralAddress);
