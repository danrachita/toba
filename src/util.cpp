...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Toba
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Toba
    // Mac: ~/Library/Application Support/Toba
    // Unix: ~/.toba
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Toba";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "toba.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "tobad.pid"));
